...
import '../../ScheduledTransferScreens/ScheduleRecipientIdemDetailPage.dart';
import '../../constance/customTextfield/uppercase_textfield.dart';
import '../../model/MfsbanklistBycountryiso2Response.dart';
import '../../model/juba_bank_list_response.dart';
...
class BankAccountNumber extends StatefulWidget {


  BankAccountNumber({Key? key,}) : super(key: key);

  @override
  State<BankAccountNumber> createState() => _BankAccountNumberState();
}

class _BankAccountNumberState extends State<BankAccountNumber> {
  ///Textfield contrller
  bool ischeck = false;
  TextEditingController ibanController = TextEditingController();
  FocusNode ibanFocus = FocusNode();
  MfsbanklistBycountryiso2Response mfsbanklistBycountryiso2Response = new MfsbanklistBycountryiso2Response();
  JubabanklistBycountryiso2Response jubabanklistBycountryiso2Response = new JubabanklistBycountryiso2Response();
  ...
  Banklist banklistModel = new Banklist();
  JubaBanklist jubaBanklistModel = new JubaBanklist();
  ...
  Future<void> loadPref() async {
    ...
    partnerPaymentMethod = sharedPreferences.getString("partnerPaymentMethod").toString();

    print("partnerPaymentMethod>>> " + partnerPaymentMethod);
    ...
    partnerPaymentMethod == "juba" ? jubabanklistBycountryiso2Api(context): mfsbanklistBycountryiso2Api(context);
    setState(() {});
  }
              ...
              onTap: () {
                ...
                if(partnerPaymentMethod == "mfs"){
                  if(selectedMFSBankCode.isEmpty){
                    ...
                    addRecipientBankAccountapi(context, accountNumber, accountType, bankName,accountHolder);
                  }
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
                        ...
                        child: DropdownButton2<Banklist>(
                          ...
                          onChanged: (Banklist? newValue) {
                            setState(() {
                              ...
                              selectedMFSBankCode = banklistModel.mfsBankCode.toString();
                              selectedMFSBankName = banklistModel.bankName.toString();
                              print("selected mfsc code>>>> "+banklistModel.mfsBankCode.toString());
                              print("selected mfsc name>>>> "+banklistModel.bankName.toString());

                            });
                          },
                          items: mfsbanklistBycountryiso2Response.data!.banklist!.map((bank) {
                        ...
                        child: DropdownButton2<JubaBanklist>(
                          ...
                          onChanged: (JubaBanklist? newValue) {
                            setState(() {
                              ...
                              selectedMFSBankCode = jubaBanklistModel.jubaNominatedCode.toString();
                              selectedMFSBankName = jubaBanklistModel.bankName.toString();
                              print("selected mfsc code>>>> "+jubaBanklistModel.jubaNominatedCode.toString());
                              print("selected mfsc name>>>> "+jubaBanklistModel.bankName.toString());

                            });
                          },
                          items: jubabanklistBycountryiso2Response.data!.banklist!.map((bank) {
              ...
              partnerPaymentMethod=="mfs" || partnerPaymentMethod=="juba"?Container():
              Column(children: [
                hSizedBox3,
                Container(
                  width: double.infinity,
...
*/
  ...
  Future<void> mfsbanklistBycountryiso2Api(
    ...
    var response = await http.get(Uri.parse(AllApiService.mfsbanklistBycountryiso2URL+"?country_iso2="+p.getString("iso2").toString()),
        // body: convert.jsonEncode(request),
        headers: {
          "X-CLIENT": AllApiService.x_client,
        });
  ...
  Future<void> addRecipientBankAccountapi(BuildContext context,String account_number,String bank_acc_type,String bank_name,String account_holder) async {
    CustomLoader.ProgressloadingDialog(context, true);
    SharedPreferences p = await SharedPreferences.getInstance();
    ...
    if(p.getString("partnerPaymentMethod").toString()=="mfs"){
      p.setString("recipientReceiveBankOrMobileNo", account_number);
      p.setString("recipientReceiveBankNameOrOperatorName", selectedMFSBankName);
    ...
    if (jsonResponse['status'] == true) {
      ...
      if(partnerPaymentMethod=="juba"){
        print("bdjkdshjgh"+jsonResponse["data"]["juba_NominatedCode"].toString());
        p.setString("juba_NominatedCode", jsonResponse["data"]["juba_NominatedCode"].toString());
      }
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context)=>ReasonforSendingScreen(status: "reason_for_sending",)));
