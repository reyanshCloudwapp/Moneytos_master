...
class _ReasonforSendingScreenState extends State<ReasonforSendingScreen> {
  ...
  //  onnavigate();
                      ...
                      GridView(
                        ...
                        children:optionlistt.map((PurposeData url) {
                          return GestureDetector(
                              onTap: () async {
                                pref(url.purposeCode.toString(),url.purposeCodeDescription.toString());
                              ...
                              child:  Container(
                                  ...
                                  child:Center(child: Text(url.purposeCodeDescription.toString(),textAlign: TextAlign.center,style: TextStyle(color: MyColors.color_text,fontSize: 10,fontFamily: "s_asset/font/raleway/raleway_medium.ttf",fontWeight: FontWeight.w500),))
                              )
                          );
                        }).toList(),
                      ),
    ...
    widget.status == "already_add"?Navigator.push(context, MaterialPageRoute(builder: (_) => SelectPaymentMethodScreen(isMfs: sharedPreferences.getString("partnerPaymentMethod").toString() == "mfs"?true:false,selectedMethodScreen: 1,))):
    widget.status == "reason_for_sending"? Navigator.push(context, MaterialPageRoute(builder: (_) => SelectPaymentMethodScreen(isMfs: sharedPreferences.getString("partnerPaymentMethod").toString() == "mfs"?true:false,selectedMethodScreen: 1,))):
    widget.status == "schedule_reason_for_sending"? Navigator.push(context, MaterialPageRoute(builder: (_) => ScheduledSelectPaymentMethodScreen(isMfs: sharedPreferences.getString("partnerPaymentMethod").toString() == "mfs"?true:false,selectedMethodScreen: 1,))):
    widget.status == "schedule_already_add"? Navigator.push(context, MaterialPageRoute(builder: (_) => ScheduledSelectPaymentMethodScreen(isMfs: sharedPreferences.getString("partnerPaymentMethod").toString() == "mfs"?true:false,selectedMethodScreen: 1,))):
    null;
    setState(() {});
    }
