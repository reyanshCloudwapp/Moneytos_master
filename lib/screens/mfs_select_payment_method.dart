...
import 'package:moneytos/screens/select_location_screen/select_location_screen.dart';
...
class _SelectPaymentMethodScreenState extends State<SelectPaymentMethodScreen> {
  ...
  prefData() async {
    ...
    debugPrint(
      "payment method partner>>>>  ${sharedPreferences.getString("partnerPaymentMethod")}",
    );
    debugPrint("iso3>>>>  ${sharedPreferences.getString("iso3")}");
...
// SelectDeliveryAddMethodScreen(ismfsAndalready: true,)
        ...
        : (widget.isMfs ?? false)
            ? Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      const NewSelectRecipientHomeDetailScreen(true),
                ),
              )
            : Navigator.push(
                context,
  ...
  Future<void> countryDetailByIso3Api(BuildContext context) async {
    ...
    var response = await http.get(
      Uri.parse(
        '${Apiservices.countryDetailByIso3api}?country_iso3=$countryISO3',
      ),
