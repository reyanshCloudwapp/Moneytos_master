...
import 'package:moneytos/screens/dash_settingscreen/setting_account_setting/front_image.dart';
...
class ManagePaymentMethodScreen extends StatefulWidget {
  ...
  const ManagePaymentMethodScreen({Key? key, required this.isMfs})
      : super(key: key);

  @override
  State<ManagePaymentMethodScreen> createState() =>
...
class _ManagePaymentMethodScreenState extends State<ManagePaymentMethodScreen> {
  ...
  nextpage() {
    ischeck == 'already_add'
        ...
        : Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => const LinkNewMethodScreen()),
          );
  }
  ...
  Future<void> paymentmethodsRequest(BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => const ManageSelectPaymentMethodScreen(
            selectedMethodScreen: 0,
          ),
        ),
      );
