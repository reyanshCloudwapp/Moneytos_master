...
import 'package:moneytos/screens/ScheduledTransferScreens/scheduledselectdeliverymethod.dart';
import 'package:moneytos/utils/import_helper.dart';

import '../../model/RecipientFiealdModel.dart';

class ScheduledAddRecipientInfoScreen extends StatefulWidget {
  const ScheduledAddRecipientInfoScreen({super.key});

  @override
  State<ScheduledAddRecipientInfoScreen> createState() =>
      _ScheduledAddRecipientInfoScreenState();
}

class _ScheduledAddRecipientInfoScreenState
    extends State<ScheduledAddRecipientInfoScreen> {
...
/*  Oncallback(bool load){
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          if (p!.getString('country_isoCode3').toString() ==
                                  'IND' ||
                              p!.getString('country_isoCode3').toString() ==
                                  'PAK') {
                            if (firstName.isEmpty) {
                              ...
                              createRecipientRequest(
                                ...
                                front_images,
                                p!.getString('country_isoCode3').toString(),
                                p!.getString('phonecode').toString(),
                                phoneNumber,
                                relationship,
                                address,
                                city,
                              ...
                              debugPrint(
                                'json..${json.encode(addfieldlist)}',
                              );
                            }
                          } else if (p!
                                  .getString('country_isoCode3')
                                  .toString() ==
                              'BGD') {
                            if (firstName.isEmpty) {
                              Utility.showFlutterToast('enter first name');
            ...
            SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  p!.getString('country_isoCode3').toString() == 'IND' ||
                          p!.getString('country_isoCode3').toString() == 'PAK'
                      ? Container()
                      : Column(
                          children: [
                            Container(
  ...
  static Future<void> createRecipientRequest(
      ...
      response.stream.transform(utf8.decoder).listen((event) {
        ...
        if (map['status'] == true) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const ScheduledBankAccountNumber(),
            ),
          );
  ...
  Future<void> createRecipient2Request(
    BuildContext context,
    String firstName,
    String lastName,
    String profileImg,
      ...
      response.stream.transform(utf8.decoder).listen((event) {
        ...
        if (map['status'] == true) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const ScheduledSelectDeliveryMethodScreen(),
            ),
          );
...
class ScheduledAddRecipientInfoScreen extends StatefulWidget{
  @override
  State<ScheduledAddRecipientInfoScreen> createState() => _ScheduledAddRecipientInfoScreenState();
}

class _ScheduledAddRecipientInfoScreenState extends State<ScheduledAddRecipientInfoScreen> {
