...
/*  Oncallback(bool load){
            ...
            GestureDetector(
              onTap: () {
                ...
                if (p!.getString('partnerPaymentMethod').toString() == 'mfs' ||
                  ...
                  if (p!.getString('country_isoCode3').toString() == 'IND' ||
                  ...
                  } else if (p!.getString('country_isoCode3').toString() ==
                      'BGD') {
                    if (first_name.isEmpty) {
                      Utility.showFlutterToast('enter first name');
                    } else if (last_name.isEmpty) {
              ...
              child: Column(
                children: [
                  ...
                  p!.getString('partnerPaymentMethod').toString() == 'mfs' ||
                          p!.getString('partnerPaymentMethod').toString() ==
                              'juba'
                      ? Container()
                      : Column(
                          children: [
  ...
  Future<void> createRecipientRequest(
    ...
    try {
      ...
      if (context != null && first_name != null && last_name != null) {
        ...
        request.fields['mobile_operator'] =
            pre.getString('mfs_mobile_operator_name').toString();
        request.fields['delivery_method_type'] =
            pre.getString('select_payment_method_status').toString();
        if (p!.getString('partnerPaymentMethod').toString() == 'juba' &&
            p!.getString('select_payment_method_status').toString() ==
                'Mobile') {
          request.fields['juba_NominatedCode'] =
              pre.getString('juba_NominatedCode').toString();
        }
      }
      ...
      response.stream.transform(utf8.decoder).listen((event) {
        ...
        if (map['status'] == true) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => (countryIso3Code == 'SOM' &&
                      (isMfsMobileMoney))
                  ? const ReasonforSendingScreen(
                      status: 'reason_for_sending',
                    )
                  : (isMfsMobileMoney) ||
                          (pre.getString('partnerPaymentMethod').toString() ==
                                  'juba' &&
                              pre
                                      .getString(
                                      ...
                                      .toString() ==
                                  'Cash')
                      ? /*SelectPaymentMethodScreen(isMfs: (widget.isMfsMobileMoney ?? false),selectedMethodScreen: 0,)*/ const ReasonforSendingScreen(
                          status: 'reason_for_sending',
                        )
          ...
          if ((pre.getString('partnerPaymentMethod').toString() == 'juba' ||
                  pre.getString('partnerPaymentMethod').toString() == 'mfs') &&
              pre.getString('select_payment_method_status').toString() ==
                  'Mobile') {
            debugPrint('save in mfs mobile case');
